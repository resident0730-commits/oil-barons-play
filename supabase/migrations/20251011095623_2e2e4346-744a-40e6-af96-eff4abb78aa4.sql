-- Обновляем функцию apply_promo_code чтобы требовать invoice_id
...
BEGIN
  -- Проверяем что invoice_id передан
  IF p_invoice_id IS NULL THEN
    RETURN jsonb_build_object('success', false, 'error', 'Промокод можно применить только при пополнении баланса');
  END IF;

  -- Проверяем что инвойс существует и принадлежит пользователю
  IF NOT EXISTS (
    SELECT 1 FROM public.payment_invoices
    WHERE id = p_invoice_id
      AND user_id = p_user_id
  ) THEN
    RETURN jsonb_build_object('success', false, 'error', 'Неверный инвойс');
  END IF;
